...
def get_idea_prompt() -> str:
    ...
    return f"""Сгенерируй микро-бизнес идею которую можно запустить за выходные.

Контекст: {category}, решает {pain}, тренд {trend}

Формат (без звёздочек, простой текст, используй эмодзи как указано):

💡 ИДЕЯ: [название 1-2 слова на английском]

🎯 ПРОБЛЕМА: [какую боль решает, 1 предложение]

⚙️ КАК РАБОТАЕТ: [3 пункта через запятую]

💰 МОНЕТИЗАЦИЯ: [как заработать, 1 предложение]

🚀 ПЕРВЫЙ ШАГ: [конкретное действие на сегодня]

Пиши кратко, по делу, без воды.""", category
...
def get_toxic_caption(idea_text: str) -> str:
    ...
    return f"{idea_text}{random.choice(outros)}"
