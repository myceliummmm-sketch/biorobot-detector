        ...
        try:
            ...
            if response.generated_images:
                ...
                if hasattr(image, 'image') and hasattr(image.image, 'image_bytes'):
                    logger.info("Image generated successfully with Imagen 3")
                    return image.image.image_bytes

            logger.warning("No image in Imagen response")
            return None

        except ImportError:
            logger.warning("google-genai not installed, trying legacy API")
            return await self._generate_image_legacy(image_prompt)
        except Exception as e:
            logger.error(f"Imagen error: {e}, trying legacy")
            return await self._generate_image_legacy(image_prompt)

    async def _generate_image_legacy(self, image_prompt: str) -> Optional[bytes]:
        """Fallback генерация через Gemini 2.0"""
        try:
            ...
            logger.error(f"Legacy image gen error: {e}")
            return None
